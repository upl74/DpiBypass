...
import io.github.dovecoteescapee.byedpi.utility.LocalSocksPort
    ...
    companion object {
        fun fromSharedPreferences(
            ...
            when (preferences.getBoolean("byedpi_enable_cmd_settings", false)) {
                true -> {
                    val cmd = preferences.getStringNotNull("byedpi_cmd_args", "")
                    val patched = socksPort?.let { LocalSocksPort.patchCmdPort(cmd, it) } ?: cmd
                    ByeDpiProxyCmdPreferences(patched)
                }
                false -> ByeDpiProxyUIPreferences(preferences, socksPort)
            }
    }
}
...
class ByeDpiProxyUIPreferences(
    ...
    constructor(preferences: SharedPreferences, portOverride: Int? = null) : this(
        ip = preferences.getString("byedpi_proxy_ip", null),
        port = portOverride ?: preferences.getString("byedpi_proxy_port", null)?.toIntOrNull(),
