...
from .paths import WINWS_EXE
from .zapret_presets import default_preset_name
from .zapret_runtime import launch_preset_bat

CREATE_NO_WINDOW = 0x08000000
_WINWS_START_TIMEOUT_S = 12.0
...
class WinWsService:
    ...
    def start(self, preset_name: str | None = None) -> None:
        ...
        self._launcher = launch_preset_bat(name)

        if not _wait_for_winws():
            code = self._launcher.poll() if self._launcher else None
            self._launcher = None
            raise RuntimeError(
                f"winws.exe не запустился ({name}).\n"
                "Нажмите «Компоненты» — обновить zapret."
